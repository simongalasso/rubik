...
impl Cubie {
    pub fn new(rubik: &mut SceneNode, size: f32, pos: Vector3<f32>, scale: f32, gap: f32) -> Cubie {
        ...
        add_facelet(&mut cubie, size - gap, Some(rot), Some(Translation3::new(0.0, r, 0.0)), color);
        // cubie R
        ...
        add_facelet(&mut cubie, size - gap, Some(rot), Some(Translation3::new(-r, 0.0, 0.0)), color);
        // cubie F
        let color: (f32, f32, f32) = if pos.z == -1.0 { C_WHITE } else { C_BLACK };
        add_facelet(&mut cubie, size - gap, None, Some(Translation3::new(0.0, 0.0, -r)), color);
        // cubie D
